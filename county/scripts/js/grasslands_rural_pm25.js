...
const colorScale = chroma.scale(['#a35866', '#ed0e38']).domain([0, 0.47]);
...
fetch('data/grasslandsRuralPM25.json')
      ...
      .then(geojsonData => {
        L.geoJson(geojsonData, {
          ...
          onEachFeature: function (feature, layer) {
            ...
            const data = countyData[fips];
            if (data) {
              layer.bindPopup(`<strong>${data.name}</strong><br>PM25 Sequestration: ${data.value} g/m²`);
            } else {
...
const grades = [0, 0.15, 0.30, 0.45, 0.60]; 
const labels = [];
...
div.innerHTML = `<strong>PM2.5 Sequestration<br>(g/m²)</strong><br>` + labels.join('<br>');
return div;
};
